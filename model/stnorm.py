...
class stnorm(nn.Module):
    ...
    def forward(self, input):
        ...
        out = out[:, :, :, 0].permute(0, 2, 1).reshape(-1, self.num_nodes, int(out.shape[1]/3), len(self.quantiles)).permute(0,2,1,3)
        # out = out * std + mean
        return out
...
def main():
    ...
    model =  stnorm(device, N_NODE, tnorm_bool=True, snorm_bool=True, in_dim=1,out_dim=TIMESTEP_OUT, channels=32, kernel_size=2, blocks=1, layers=4).to(device)
