...
class DCGRU_Cell2(nn.Module):
    def __init__(self, num_nodes: int, input_dim: int, hidden_dim: int, K: int, bias=True, activation=None):
        super(DCGRU_Cell2, self).__init__()
        self.num_nodes = num_nodes
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
...
class DCGRU_Encoder(nn.Module):
    def __init__(self, num_nodes: int, input_dim: int, hidden_dim, K: int, num_layers: int,
        ...
        for i in range(self.num_layers):
            ...
            self.cell_list.append(DCGRU_Cell1(num_nodes=num_nodes,
...
class DCGRU_Decoder(nn.Module):  # projected output as input at the next timestep
    def __init__(self, num_nodes: int, out_horizon: int, out_dim: int, hidden_dim, K: int, num_layers: int,
        ...
        for i in range(self.num_layers):
            ...
            self.cell_list.append(DCGRU_Cell2(num_nodes=num_nodes,
                                             input_dim=cur_input_dim,
                                             hidden_dim=self.hidden_dim[i],
                                             K=K,
                                             bias=bias,
...
class dcrnn3(nn.Module):
    def __init__(self, quantile, device, num_nodes: int, input_dim: int, out_horizon: int, P: list, K=3, hidden_dim=64, num_layers=2, bias=True,
                 activation=None):
        super(dcrnn3, self).__init__()
        self.quantile = quantile
        self.start_conv = nn.Conv2d(in_channels=input_dim,
                                    out_channels=len(quantile),
                                    kernel_size=(1,1))
        self.K = K
        ...
        self.decoder = DCGRU_Decoder(num_nodes=num_nodes, out_dim=len(quantile), hidden_dim=hidden_dim, K=self.P.shape[0],
    ...
    def forward(self, x_seq: torch.Tensor):
        ...
        '''
        assert len(x_seq.shape) == 4, 'DCGRU must take in 4D tensor as input x_seq'
        # x_seq = self.start_conv(x_seq.permute(0,3,2,1)).permute(0,3,2,1)
        # print('x_seq shape :', x_seq.shape)
        # encoding
        _, h_t_lst = self.encoder(P=self.P, x_seq=x_seq,
                                  h_0_l=None)  # encoder returns layerwise last hidden state [(B, N, C)] * L
        #print('_ shape :',_[-1].shape)  # [8, 12, 207, 64]
        # print('h_t_lst[0] shape :',h_t_lst[0].shape)  # [8, 207, 64]
        # decoding
        # deco_input = self.decoder.out_projector(h_t_lst[-1])        # initiate decoder input
        deco_input = torch.zeros((x_seq.shape[0], x_seq.shape[2], len(self.quantile)),
        ...
        # print('deco single out shape :',output.shape)  # [8, 207, 1]
...
def main():
    ...
    ADJPATH = '../data/metr-la/adj_mx.pkl'
    ADJTYPE = 'doubletransition'
    GPU = sys.argv[-1] if len(sys.argv) == 2 else '3'
    device = torch.device("cuda:{}".format(GPU)) if torch.cuda.is_available() else torch.device("cpu")
    adj_mx = load_adj(ADJPATH, ADJTYPE)
    model = dcrnn3(QUANTILES, device, num_nodes=207, input_dim=CHANNEL, out_horizon=TIMESTEP_OUT, P=adj_mx).to(device)
    # summary(model, (TIMESTEP_IN, 207, CHANNEL), device=device)
    x=torch.randn((8,12,207,1)).to(device)
    out = model(x)
