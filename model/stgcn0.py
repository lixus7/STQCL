...
class output_layer(nn.Module):
    def __init__(self, c, T, n, quantiles):
...
class stgcn0(nn.Module):
    def __init__(self, ks, kt, bs, T, n, Lk, p, quantiles):
        super(stgcn0, self).__init__()
        self.st_conv1 = st_conv_block(ks, kt, n, bs[0], p, Lk)
        self.st_conv2 = st_conv_block(ks, kt, n, bs[1], p, Lk)
        self.output = output_layer(bs[1][2], T - 4 * (kt - 1), n, quantiles)
    ...
    def forward(self, x):
        ...
        # print('out shape is : ',self.output(x_st2).permute(0,1,3,2).shape)
        return self.output(x_st2).permute(0, 1, 3, 2)
...
def main():
    ...
    model = stgcn0(ks, kt, bs, T, n, Lk, p, QUANTILES).to(device)
