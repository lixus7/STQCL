...
class output_layer(nn.Module):
    def __init__(self, c, T, n, quantiles, TIMESTEP_OUT):
        ...
        # 一次输出12 * 3个
        self.fc = nn.Conv2d(c, 12, 1)
        self.quantile_output = nn.Conv2d(1,len(quantiles),1)
    ...
    def forward(self, x):
        ...
        out1 = self.fc(x_t2)   
#         print("out1 shape is:", out1.shape)     # out1 shape is: torch.Size([8, 12, 1, 207])

        out2 = self.quantile_output(out1.permute(0,2,1,3)).permute(0,2,3,1)
#         print("out2 shape is:", out2.shape)     # out1 shape is: torch.Size([8, 12, 1, 207])
#         exit()    
        return out2


class stgcn2(nn.Module):
    def __init__(self, ks, kt, bs, T, n, Lk, p, quantiles, TIMESTEP_OUT):
        super(stgcn2, self).__init__()
        self.st_conv1 = st_conv_block(ks, kt, n, bs[0], p, Lk)
        self.st_conv2 = st_conv_block(ks, kt, n, bs[1], p, Lk)
        self.output = output_layer(bs[1][2], T - 4 * (kt - 1), n, quantiles, TIMESTEP_OUT)
...
def main():
    ...
    model = stgcn2(ks, kt, bs, T, n, Lk, p, QUANTILES, TIMESTEP_OUT).to(device)
