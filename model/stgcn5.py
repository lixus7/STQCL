...
class stgcn5(nn.Module):
    def __init__(self, ks, kt, bs, T, n, Lk, p, adj, k, quantiles, TIMESTEP_OUT, stdrop, sort_rank, s_epoch, cl_epoch):  
        super(stgcn5, self).__init__()
        self.st_conv1 = st_conv_block(ks, kt, n, bs[0], p, Lk)
        self.st_conv2 = st_conv_block(ks, kt, n, bs[1], p, Lk)
        self.output1 = output_layer(bs[1][2], T - 4 * (kt - 1), n, quantiles, TIMESTEP_OUT)
        # self.output2 = output_layer(bs[1][2], T - 4 * (kt - 1), n, quantiles, TIMESTEP_OUT)
        adj[adj > 1] = 1
        A = adj.clone()
        for i in range(3):
            A += adj.matmul(adj)
        A[A > 1] = 1
        self.A = A
        self.dropout = STDrop(stdrop, sort_rank, s_epoch, cl_epoch, A, k=k)

    def forward(self, x, mean, std, y=None, p=None, c_epoch=None):
        # x : b,t,n,c
        x = x.permute(0, 3, 1, 2)
        x_st1 = self.st_conv1(x)
        x_st2 = self.st_conv2(x_st1)
        output_1 = self.output1(x_st2)
        output_1 = output_1.reshape(-1, 12, 3, output_1.shape[3]).permute(0, 1, 3, 2)
        pred_y = output_1 * std + mean
        if self.training:
            x_st2, self.score = self.dropout(x_st2, pred_y, y, p, c_epoch)  
        output_1 = self.output1(x_st2)
        # print("output1 shape is:", output_1.shape)
        output_2 = output_1.reshape(-1, 12, 3, output_1.shape[3]).permute(0, 1, 3, 2)
        # print("output2 shape is:", output_2.shape)
        output = output_2 * std + mean
        return output  # [b,t=36,c=1,n] -> [b,t=12,n,q=3]  torch.reshape
...
def main():
    ...
    ADJPATH = '../data/metr-la/W_metrla.csv'
    ...
    model = stgcn3(ks, kt, bs, T, n, Lk, p, QUANTILES, TIMESTEP_OUT).to(device)
