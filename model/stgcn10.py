...
class stgcn7(nn.Module):
    def __init__(self, ks, kt, bs, T, n, Lk, p, adj, k, quantiles, TIMESTEP_OUT):  
        super(stgcn7, self).__init__()
    ...
    def forward(self, x, mean, std, y=None, p=None, mask_rank=None):
        # x : b,t,n,c
        x = x.permute(0, 3, 1, 2)
...
#         pred_y = output_1 * std + mean
        if self.training and mask_rank!=None :
            x_st2, self.score = self.dropout(x_st2, mask_rank)  
        output_1 = self.output1(x_st2)
...
def main():
    ...
    model = stgcn7(ks, kt, bs, T, n, Lk, p, QUANTILES, TIMESTEP_OUT).to(device)
