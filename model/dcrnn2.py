...
class dcrnn2(nn.Module):
    def __init__(self, quantile, device, num_nodes: int, input_dim: int, out_horizon: int, P: list, K=3, hidden_dim=64, num_layers=2, bias=True,
        ...
        self.encoder = DCGRU_Encoder(num_nodes=num_nodes, input_dim=len(quantile), hidden_dim=hidden_dim, K=self.P.shape[0],
...
def main():
    ...
    model = dcrnn2(QUANTILES, device, num_nodes=207, input_dim=CHANNEL, out_horizon=TIMESTEP_OUT, P=adj_mx).to(device)
