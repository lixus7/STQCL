...
class DCGRU_Decoder(nn.Module):  # projected output as input at the next timestep
    def __init__(self, num_nodes: int, out_horizon: int, input_dim: int, out_dim: int, hidden_dim, K: int,
                 num_layers: int,
        ...
        self.out_projector = nn.Linear(in_features=self.hidden_dim[-1], out_features=self.out_dim, bias=bias)
...
class dcrnn1(nn.Module):
    def __init__(self, device, num_nodes: int, input_dim: int, out_horizon: int, P: list, quantiles, K=3, hidden_dim=64,
                 num_layers=2, bias=True,
                 activation=None):
        super(dcrnn1, self).__init__()
        self.K = K
        self.device = device
        self.P = self.compute_cheby_poly(P).to(self.device)
        ...
        self.decoder = DCGRU_Decoder(num_nodes=num_nodes, input_dim=input_dim, out_dim=len(quantiles),
                                     hidden_dim=hidden_dim, K=self.P.shape[0], num_layers=num_layers, bias=bias,
                                     activation=activation, out_horizon=out_horizon)
    ...
    def forward(self, x_seq: torch.Tensor):
        ...
        for t in range(self.decoder.out_horizon):
            ...
            deco_input = output.index_select(2, torch.tensor([output.shape[2] // 2]).to(self.device))  # update decoder input
            outputs.append(output.unsqueeze(1))

        outputs = torch.cat(outputs, dim=1)  # (B, horizon, N, C)
        # print('1 out shape : ',outputs.shape)
        print('out shape : ',outputs.permute([0, 3, 2, 1]).shape)
        return outputs # (B, C, N, H)
...
def main():
    ...
    model = dcrnn1(device, num_nodes=207, input_dim=CHANNEL, out_horizon=TIMESTEP_OUT, P=adj_mx,
