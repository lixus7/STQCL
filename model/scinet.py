    ...
    def __init__(self, output_len, input_len, input_dim = 9, hid_size = 1, num_stacks = 1,
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                m.bias.data.zero_()
        self.projection1 = nn.Conv1d(self.input_len, self.output_len*3, kernel_size=1, stride=1, bias=False)
...
#             output = x.reshape(x.shape[0],self.output_len,3,x.shape[2]).permute(0,1,3,2) * std + mean
            output = x.reshape(x.shape[0],self.output_len,3,x.shape[2]).permute(0,1,3,2) 
            return output
