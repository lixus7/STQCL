...
class scinet_point(nn.Module):
    def __init__(self, output_len, input_len, input_dim = 9, hid_size = 1, num_stacks = 1,
        ...
        super(scinet_point, self).__init__()
...
#             print('/// RIN ACTIVATED ///\r',end='')
        ...
        if self.stacks == 1:
            ...
            if self.RIN:
                ...
                x = x * stdev
                x = x + means
            #print('x3 shape is :',x.reshape(x.shape[0],self.output_len,3,x.shape[2]).permute(0,1,3,2).shape)
            #exit()
#             output = x.reshape(x.shape[0],self.output_len,3,x.shape[2]).permute(0,1,3,2) * std + mean
            output = x.reshape(x.shape[0],self.output_len,1,x.shape[2]).permute(0,1,3,2) 
            return output
...
if __name__ == '__main__':
    ...
    model = scinet_point(output_len = args.horizon, input_len= args.window_size, input_dim = 9, hid_size = args.hidden_size, num_stacks = 1,
