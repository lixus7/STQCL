...
parser.add_argument('-model', type=str, default='stgcn_point', help='choose which model to train and test')
...
parser.add_argument('-clip_grad', type=int, default=0, help='clip grad size')
...
parser.add_argument('-loss', type=str, default='masked_mae', help='loss function, combine1, combine2, combine3')
...
def data_preprocess(whichdata):
    ...
    if whichdata == 'pems03':
        data = np.load('./data/PEMS03/PEMS03.npz')['data'][:, :, 0]  # [samples,nodes]
        start = '2018-09-01 00:00:00'
        end = '2018-11-30 23:59:00'
        freq = '300s'
        pdates = pd.date_range(start=start, end=end, freq=freq)
        print('pdate shape is: ', pdates.shape)
        time = pd.DataFrame(pdates, columns=['date'])
    if whichdata == 'pems07':
        data = np.load('./data/PEMS07/PEMS07.npz')['data'][:, :, 0]  # [samples,nodes]
        start = '2017-05-01 00:00:00'
        end = '2017-8-06 23:59:00'
        freq = '300s'
        pdates = pd.date_range(start=start, end=end, freq=freq)
        print('pdate shape is: ', pdates.shape)
        time = pd.DataFrame(pdates, columns=['date'])
    if whichdata == 'pems08':
        data = np.load('./data/PEMS08/PEMS08.npz')['data'][:, :, 0]  # [samples,nodes]
        start = '2016-07-01 00:00:00'
        end = '2016-8-31 23:59:00'
        freq = '300s'
        pdates = pd.date_range(start=start, end=end, freq=freq)
        print('pdate shape is: ', pdates.shape)
        time = pd.DataFrame(pdates, columns=['date']) 
    ...
    if whichdata == 'metrla':
        ...
        data = data.values
    if whichdata == 'pemsbay':
        data = pd.read_hdf('./data/pemsbay/pems-bay.h5')
        data.index.freq = data.index.inferred_freq
        time = pd.DataFrame(data.index, columns=['date'])
        data = data.values        
...
def getModel(name, device):
    ...
    if args.model == 'stgcn_point':
        ...
        Lk = baseline_py_file.cheb_poly(L, ks)
        Lk = torch.Tensor(Lk.astype(np.float32)).to(device)
        model = baseline_py_file.stgcn_point(ks, kt, bs, T, n, Lk, p, quantiles, TIMESTEP_OUT).to(device)
    if args.model == 'gwn0':
        ...
        model = baseline_py_file.gwn0(device, num_nodes=N_NODE, in_dim=CHANNEL, supports=None,
    ...
    if args.model == 'gwn4':
        ...
        model = baseline_py_file.gwn4(device, quantiles=quantiles, num_nodes=N_NODE, in_dim=CHANNEL, supports=None,
                                      layers=2).to(device)
    if args.model == 'dcrnn0':
        adj_mx = baseline_py_file.load_adj(DCRNN_ADJ, ADJTYPE)
        model = baseline_py_file.dcrnn0(device, num_nodes=N_NODE, input_dim=CHANNEL, out_horizon=TIMESTEP_OUT, P=adj_mx).to(device)
    ...
    #         else:
    #             nn.init.uniform_(p)
    if args.model == 'staeformer_point':
        model = baseline_py_file.staeformer_point(num_nodes=N_NODE,in_steps=12,out_steps=12,steps_per_day=288,input_dim=3,output_dim=1, 
                                            input_embedding_dim=24,tod_embedding_dim=24,dow_embedding_dim=24,spatial_embedding_dim=0,
                                            adaptive_embedding_dim=80,feed_forward_dim=256,num_heads=4,num_layers=3,dropout=0.1,use_mixed_proj=True).to(device)
    if args.model == 'stnorm_point':
        model = baseline_py_file.stnorm_point(device, quantiles= quantiles, num_nodes= N_NODE, tnorm_bool=True, snorm_bool=True, in_dim=1,out_dim=TIMESTEP_OUT, channels=32, kernel_size=2, blocks=1, layers=4).to(device)
#####   
    if args.model == 'scinet_point':
        model = baseline_py_file.scinet_point(output_len=TIMESTEP_OUT, input_len=TIMESTEP_IN, input_dim = N_NODE, hid_size = 0.0625, num_stacks = 1,
...
def model_inference(name, model, val_iter, test_iter, x_stats, save=True, with_point=False, point_prediction=True):
    ...
    return val_rmse, val_mae, val_mape, val_smape, val_rse, test_rmse, test_mae, test_mape, test_smape, test_rse
...
def trainModel(name, device, data, x_stats):
    ...
    if OPTIMIZER == 'RMSprop':
        ...
        if args.sche:
            optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE, weight_decay=0.0001, eps=1e-8)
    # scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[40], gamma=0.1,verbose=False)   
#     if args.model =='scinet':
...
#         else:
        ...
        if epoch % PRINT_EPOCH == 0:
            ...
            # 原版STGCN的点预测
            if args.model == 'stgcn0':
                val_rmse, val_mae, val_mape, val_smape, val_rse, \
                test_rmse, test_mae, test_mape, test_smape, test_rse = model_inference(name, model,
                                                                                                       val_iter,
                                                                                                       test_iter,
                                                                                                       x_stats,
                                                                                                       save=False,
...
#         val_loss, test_loss = val_mae, test_mae
        val_loss, test_loss = val_rmse, test_rmse
...
#             val_loss, test_loss = val_quantiles, test_quantiles
    ...
    val_rmse, val_mae, val_mape, val_smape, val_rse, \
    test_rmse, test_mae, test_mape, test_smape, test_rse = model_inference(name, model, val_iter,
                                                                                           test_iter, x_stats,
                                                                                           save=False,
                                                                                           with_point=True)
    ...
    print(row % (test_rmse[2], test_mae[2], test_mape[2], 0, 0, 0,test_rmse[5], test_mae[5], test_mape[5], 0, 0, 0, test_rmse[11], test_mae[11], test_mape[11], 0, 0,
                 0))
...
def multi_version_test(name, device, train, versions):
    ...
    if LOSS == 'combined':
        ...
        with_point = True
        model = Model(model, quantiles).to(device)        
    rmse_all, mae_all, mape_all, smape_all, rse_all = np.zeros(
        ...
        (len(train), len(versions), TIMESTEP_OUT)), np.zeros((len(train), len(versions), TIMESTEP_OUT))  # [V,T]
    #     val_iter = torch_data_loader(device, data, data_type='val', shuffle=True)
    #     test_iter = torch_data_loader(device, data, data_type='test', shuffle=False)
    ...
    print(head % ("=", "=", "Horizon", "3", "=", "=", "=", "=", "Horizon", "6", "=", "=",  "=", "=", "Horizon", "12", "=", "="))
    print(head % ("RMSE", "MAE", "MAPE", "Q10", "Q50", "Q90","RMSE", "MAE", "MAPE", "Q10", "Q50", "Q90","RMSE", "MAE", "MAPE", "Q10", "Q50", "Q90"))
    print(row % (rmse[2], mae[2], mape[2], 0, 0, 0,rmse[5], mae[5], mape[5], 0, 0, 0, rmse[11], mae[11], mape[11], 0, 0, 0))    
...
def main():
    ...
    if args.mode == 'all':
        ...
        multi_version_test(MODELNAME, device, train=[0.7], versions=[str(i) for i in np.arange(0, 5)])  #
